...
<?php 
if (empty($_POST['author']) AND empty($_POST['title'])
		AND empty($_POST['category']) AND empty($_POST['user'])) {
			echo  'Inserire un campo per la ricerca. ' ;
		} else
$items = Libro::getSearchlibro();
if (empty($items)) {
  $items = array();
};
